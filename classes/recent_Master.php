...
Class Master extends DBConnection {
    ...
    function save_student(){
        ...
        // Loop through POST data and add to $data, but skip 'id' and 'date_created'
        foreach($_POST as $k => $v){
            if(!in_array($k, array('id', 'date_created'))){  // Skip 'id' and 'date_created'
                if(!empty($data)) $data .=",";
                $data .= " `{$k}`='{$v}' ";  // Add the data field to the SQL query
            }
        }
        ...
        if($check > 0){
            $resp['status'] = 'failed';
            $resp['msg'] = "Student Name already exists.";
            return json_encode($resp);
            exit;
        }

        if(empty($id)){
            // For new student, the `date_created` will automatically be set to CURRENT_TIMESTAMP by MySQL
            $sql = "INSERT INTO `student_list` SET {$data}";
            $save = $this->conn->query($sql);
        } else {
            // For existing student, we need to make sure date_updated is also set
            $sql = "UPDATE `student_list` SET {$data}, `date_updated` = CURRENT_TIMESTAMP WHERE id = '{$id}'";
            $save = $this->conn->query($sql);
        }
