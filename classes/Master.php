...
// Check if there's an ID (if it's an update or insert)
        if (empty($id)) {
            // Insert query
            $sql = "INSERT INTO `scholar_history_list` 
            (`user_id`, `individual_id`, `scheme_id`, `location_id`, `disbursement_status`, `reviewed_by`, `remarks`, `date_created`) 
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())"; // NOW() for current timestamp
        } else {
            // Update query
            $sql = "UPDATE `scholar_history_list` 
            SET 
            `user_id` = ?, 
            `individual_id` = ?, 
            `scheme_id` = ?, 
...
// Prepare the statement
        if ($stmt = $this->conn->prepare($sql)) {
            ...
            if (empty($id)) {
                // If it's an insert, bind the values
                $stmt->bind_param('iiiisss',
                    $_POST['user_id'],
                    $_POST['individual_id'],
                    $_POST['scheme_id'],
                    $_POST['location_id'],
                    $_POST['disbursement_status'],
                    $_POST['reviewed_by'],
                    $_POST['remarks']
                );
            } else {
                // If it's an update, bind the values + ID for WHERE clause
                $stmt->bind_param('iiiisssi',
                    ...
                    $_POST['reviewed_by'],
                    $_POST['remarks'],
                    $id // Add the ID for the WHERE clause
                );
            }
            ...
            if ($stmt->execute()) {
                // Success response
                $resp['status'] = 'success';
                $this->settings->set_flashdata('success', empty($id) ? "New record saved." : "Record updated.");
            } else {
                // Error response
                $resp['status'] = 'failed';
                $resp['err'] = "Error executing query: " . $stmt->error;
                return json_encode($resp);
            }
        }


        if (isset($_POST['scheme_id']) && isset($_POST['individual_id'])) {

            $scheme_id = $_POST['scheme_id']; // Get the selected scheme_id from the form
            $individual_id = $_POST['individual_id']; // Get the individual ID
            error_log("Received scheme_id: " . $scheme_id);
            error_log("Received individual_id: " . $individual_id);
            // Prepare the SQL to update the scheme_id for the individual
            ...
            if ($stmt2 = $this->conn->prepare($sql2)) {
                ...
                if ($stmt2->execute()) {
                    if ($stmt2->affected_rows > 0) {
                        echo json_encode(['status' => 'success', 'msg' => 'Scheme updated successfully']);
                    } else {
                        echo json_encode(['status' => 'success', 'msg' => 'No changes to be made']);
                    }
                } else {
                    echo json_encode(['status' => 'failed', 'msg' => 'Error updating scheme: ' . $stmt2->error]);
                }
                $stmt2->close();
            } else {
                echo json_encode(['status' => 'failed', 'msg' => 'Error preparing query: ' . $this->conn->error]);
            }
        } else {
            echo json_encode(['status' => 'failed', 'msg' => 'Missing data']);
        }
        // Return response
        return json_encode($resp);
    }
