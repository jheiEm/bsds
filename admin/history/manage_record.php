            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    <select name="scheme_id" id="scheme_id" class="custom-select custom-select-sm select2" required>
                        ...
                        <?php
                        // Query active schemes
                        $vaccine_qry = $conn->query("SELECT * FROM scheme_list WHERE `status` = 1 ORDER BY `id` ASC");
                        while($row = $vaccine_qry->fetch_assoc()):
                            ?>
                            <option value="<?php echo $row['id']; ?>" <?php echo isset($scheme_id) && $scheme_id == $row['id'] ? 'selected' : ''; ?>>
                                <?php echo $row['name']; ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                ...
                <div class="col-md-6">
                    <div class="mb-2">
                        ...
                        <select name="disbursement_status" id="disbursement_status" class="custom-select select" required>
                            ...
                            <option <?php echo isset($disbursement_status) && $disbursement_status == 'Approved for Disbursement' ? 'selected' : '' ?>>Approved for Disbursement</option>
                        </select>
                    </div>
                    <div class="">
    ...
    $(document).ready(function(){
        ...
        $('#history-form').submit(function(e){
            ...
            $.ajax({
                ...
                error:err=>{
                     //console.log(err)
                     //alert_toast("An error occurred DIOOO",'error');
                     //end_loader();
                ...
                success:function(resp){
                    if(!!resp.id){
                        ...
                        if (resp.status == 0) {
                            ...
                            $('dd.indi_status').text('Approved for Disbursement');
