        ...
        <div class="container-fluid">
            <div class="container-fluid">
                <table class="table table-bordered table-striped">
                    ...
                    $i = 1;
                    $where = "";
                    $location_id = $_settings->userdata('location_id'); // Assuming $_settings->userdata('location_id') gives the logged-in user's location_id

                    if($_settings->userdata('type') != 1){
                        $where = " where h.district_name = '{$_settings->userdata('location_id')}'";
                    }
                    ...
                    $qry = $conn->query("SELECT h.*,concat(i.lastname,', ',i.firstname,' ', i.middlename) as iname,i.tracking_code FROM `scholar_history_list` h inner join individual_list i on h.individual_id = i.id {$where} order by unix_timestamp(h.date_created) desc");
                    while($row = $qry->fetch_assoc()):
                        ...
                        <tr>
                            ...
                            <td class="py-1 px-2">
                                <small><span class="text-muted">Disbursement:</span> <b><?php echo isset($vax_arr[$row['scheme_id']]) ? $vax_arr[$row['scheme_id']] : 'Scheme was Deleted' ; ?></b></small> <br>
                                <small><span class="text-muted">Status:</span> <b><?php echo $row['disbursement_status'] ?></b></small>
...
<script>
    $(document).ready(function(){
        $('.delete_data').click(function(){
            _conf("Are you sure to delete this Record permanently?","delete_history",[$(this).attr('data-id')])
        })
