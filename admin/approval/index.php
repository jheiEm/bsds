...
<div class="card card-outline card-primary">
    ...
    <div class="card-body">
        <div class="container-fluid">
            ...
            <!-- Mass Update Button -->
            <button id="massUpdateBtn" class="btn btn-success">Approve for Disbursement</button>
        </div>
    </div>
</div>
...
<!--                        <input type="text" class="form-control" id="editEmail" name="email">-->
                        <input type="text" class="form-control" id="editEmail" name="email" value="<?= isset($row['email']) ? $row['email'] : '' ?>">
                    ...
                    <div class="mb-3">
                        <label for="editScheme" class="form-label">Scheme</label>
                    ...
                    <!-- Disbursement Schedule Dropdown -->
                    <div class="mb-3">
                        <label for="editSchedule" class="form-label">Disbursement Schedule</label>
                        <select class="form-select" id="editSchedule" name="scheduleId">
                            <option value="">Select Disbursement Schedule</option>

                            <?php
                            // Query to fetch unique schedule_date values from disbursement_schedule, ignoring time part
                            $scheduleResult = $conn->query("SELECT DISTINCT DATE(schedule_date) AS schedule_date FROM disbursement_schedule WHERE status = 1");

                            // Create an array to track schedule dates that have already been added
                            $addedScheduleDates = [];

                            while ($schedule = $scheduleResult->fetch_assoc()) {
                                // Extract only the date part (YYYY-MM-DD) from the schedule_date field
                                $formattedScheduleDate = $schedule['schedule_date']; // This is already the date part

                                // Check if this schedule date has already been added to avoid duplicates
                                if (!in_array($formattedScheduleDate, $addedScheduleDates)) {
                                    // Mark this date as added
                                    $addedScheduleDates[] = $formattedScheduleDate;

                                    // Display the schedule date option
                                    echo "<option value='{$formattedScheduleDate}'>{$formattedScheduleDate}</option>";
                                }
                            }
                            ?>
                        </select>
...
<script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (data.success) {
                                ...
                                $('#editScheme').val(data.individual.scheme_id);
                                $('#editEmail').val(data.individual.email);
                               // $('#editSchedule').val(data.individual.scheduleId);

                                // Set the value of the schedule field
                                var schedule = data.individual.schedule; // The schedule field (could be a date or scheduleId)
                                if (schedule && schedule !== '0000-00-00') {
                                    $('#editSchedule').val(schedule); // Set the schedule if it's a valid date
                                } else {
                                    $('#editSchedule').val(''); // Set to blank or default value if no valid schedule
                                }
    ...
    $('#massUpdateBtn').on('click', function() {
        ...
        var newStatus = prompt("Enter new status (0: Pending, 1: Approved for Disbursement, 2: Cancelled):");
        ...
        if (newStatus === '1') {
            var disbursementDate = prompt("Enter schedule of disbursement date (YYYY-MM-DD):");
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { ids: selectedIds, status: newStatus, scheduleId: disbursementDate },
            success: function(response) {
                alert(response);
